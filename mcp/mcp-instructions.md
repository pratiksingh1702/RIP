This is a **brilliant vision** - a self-installing MCP that auto-detects AI agents, configures them, and sets up project instructions. Here's the complete implementation:

## `cli/commands/mcp.py` - MCP Installation & Configuration Command
...
def detect_installed_agents() -> dict[str, dict]:
    ...
    for agent_id, agent_info in AGENT_CONFIGS.items():
        ...
        if found_path:
            ...
            console.print(f"[yellow]Installed but not configured[/yellow]")
        elif cli_installed:
            console.print(f"[yellow]CLI found but no MCP config[/yellow]")
        else:
...
def mcp_install(
    repo_path: Path = typer.Option(
        Path("."), "--repo", "-r", help="Repository path to index"
    ),
    agent: Optional[str] = typer.Option(
        None, "--agent", "-a", help="Specific agent to configure (codex, claude, cursor, windsurf, aider)"
    ),
    all_agents: bool = typer.Option(
        False, "--all", help="Configure all detected agents"
    ),
    instructions_only: bool = typer.Option(
        False, "--instructions-only", help="Only update instructions files, skip MCP config"
    ),
...
def mcp_remove(
    agent: Optional[str] = typer.Option(
        None, "--agent", "-a", help="Specific agent to remove MCP config from"
    ),
    all_agents: bool = typer.Option(
        False, "--all", help="Remove from all configured agents"
    ),
) -> None:
    ...
    for agent_id, agent_info in agents_to_remove.items():
...
from cli.commands.mcp import mcp_install, mcp_status, mcp_remove
...
def mcp_install_command(
    ...
    instructions_only: bool = typer.Option(False, "--instructions-only"),
...
repo mcp install --agent codex
repo mcp install --agent claude

# Install for all agents in dry-run mode
repo mcp install --all --dry-run

# Only update instructions files (skip MCP config)
repo mcp install --instructions-only
...
repo mcp remove --agent cursor
```

This auto-detects Codex, Claude, Cursor, Windsurf, and Aider, configures their MCP settings, and writes agent-specific instructions files that tell them to prefer RIP tools over raw shell commands!