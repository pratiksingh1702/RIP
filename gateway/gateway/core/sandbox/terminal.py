    ...
    async def _run_command(self, command: str, sanitized: str) -> None:
        ...
        if self._persistent_writer is None:
            ...
            try:
                ...
                try:
                    container = await asyncio.to_thread(self._orchestrator.client.containers.get, self.sandbox_id)
                    await asyncio.to_thread(self._orchestrator._deploy_stream_agent, container)
                    self._persistent_reader, self._persistent_writer = await asyncio.wait_for(
                        asyncio.open_connection("127.0.0.1", port),
                        timeout=1.5
                    )
                    ...
                    logger.warning("[stream] stream agent recovery failed: %r", retry_e)
                    await self._run_command_legacy(command, sanitized, self._current_start_time)
                    self._is_executing = False
                    return
