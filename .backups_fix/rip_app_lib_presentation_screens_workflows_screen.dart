...
class _WorkflowsScreenState extends ConsumerState<WorkflowsScreen> {
  ...
  // Handle block tap for wire connection
  void _handleBlockTap(String stepId, {required bool isOutputTap}) {
    if (!_wireMode) {
      // Regular selection mode
      setState(() {
        if (_selectedBlockIds.contains(stepId)) {
          _selectedBlockIds.remove(stepId);
        } else {
    ...
    // Wire mode logic
    if (isOutputTap) {
      // User tapped OUT port - set as source
      setState(() {
        _wireSourceId = (_wireSourceId == stepId) ? null : stepId;
      });
    } else {
      // User tapped IN port - complete connection
      if (_wireSourceId != null && _wireSourceId != stepId) {
        _pushUndo();
        _connect(_selected!, _wireSourceId!, stepId);
