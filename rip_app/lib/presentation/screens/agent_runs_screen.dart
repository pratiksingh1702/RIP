            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  child: ListTile(
                    leading: Icon(
                      pending ? Icons.verified_user_rounded : status == 'completed' ? Icons.check_circle : Icons.sync,
                      color: pending ? Colors.amber.shade800 : status == 'completed' ? Colors.green : Colors.orange,
                    ),
...
class _AgentRunDetailSheetState extends ConsumerState<_AgentRunDetailSheet> {
  ...
  Future<Map<String, dynamic>> _load() => ref.read(ripClientProvider).getAgentRun(widget.runId);

  Future<void> _decide(bool approved) async {
    await ref.read(ripClientProvider).approveAgentTool(runId: widget.runId, approved: approved);
    widget.onChanged();
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        final pending = run['pending_approval'] is Map ? Map<String, dynamic>.from(run['pending_approval'] as Map) : null;
        return DraggableScrollableSheet(
          ...
          builder: (_, scrollController) => ListView(
            ...
            children: [
              ...
              if (run['summary'] != null) ...[
                const Divider(), Text('Summary:', style: Theme.of(context).textTheme.titleSmall), Text('${run['summary']}'),
              ],
              const Divider(), Text('Live Trace:', style: Theme.of(context).textTheme.titleSmall), const SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Approval needed', style: Theme.of(context).textTheme.titleSmall), const SizedBox(height: 6),
        Text('${pending['tool_name'] ?? pending['tool']}: $target'), const SizedBox(height: 10),
        Row(children: [
...
PipelineTrace _traceFromRun(String runId, Map<String, dynamic> run) {
  ...
  events.add(PipelineEvent(sessionId: runId, stage: 'started', status: 'ok', detail: run['query']?.toString() ?? 'Agent run', meta: const {}, seq: seq++, timestamp: startedAt, source: 'agent'));
  for (final raw in steps) {
    ...
    events.add(PipelineEvent(sessionId: runId, stage: ok ? 'tool_done' : 'tool_failed', status: ok ? 'ok' : 'failed', detail: 'Turn ${step['turn']}: $tool', meta: {'tool': tool, if (result['error'] != null) 'error': result['error']}, seq: seq++, timestamp: DateTime.tryParse(step['timestamp']?.toString() ?? '') ?? startedAt, source: 'agent'));
  }
  if (run['pending_approval'] != null) {
    final pending = Map<String, dynamic>.from(run['pending_approval'] as Map);
    events.add(PipelineEvent(sessionId: runId, stage: 'approval_needed', status: 'ok', detail: 'Waiting for approval: ${pending['tool_name'] ?? pending['tool'] ?? 'tool'}', meta: pending, seq: seq++, timestamp: DateTime.tryParse(pending['requested_at']?.toString() ?? '') ?? DateTime.now(), source: 'agent'));
  }
