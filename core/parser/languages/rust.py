...
class RustParser(BaseParser):
    ...
    def _walk_tree(
        ...
        elif node.type in ("function_item", "fn_item"):
            name_node = node.child_by_field_name("name")
            name = name_node.text.decode("utf-8") if name_node else "anonymous"
            entities.append(
                ParsedEntity(
    ...
    def _extract_imports(self, node, content: str) -> list[str]:
        imports: list[str] = []
        if node.type in ("use_declaration", "use"):
            # For simplicity, extract whole path
            imports.append(node.text.decode("utf-8").replace("use ", "").replace(";", ""))
        for child in node.named_children:
    ...
    def _module_name(self, file_path: Path) -> str:
        ...
        return ".".join(part for part in parts if part not in ("index", "main", "lib"))
